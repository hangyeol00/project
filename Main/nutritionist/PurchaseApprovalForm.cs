...
namespace nutritionist
{
    public partial class PurchaseApprovalForm : Form
    {
        ...
        private void LoadPurchaseRequests()
        {
            try
            {
                ...
                if (dgvPurchaseRequests != null)
                {
                    ...
                    ConfigureGrid(dgvPurchaseRequests);
                    SetColumnHeadersKorean(dgvPurchaseRequests);
        ...
        private static void SetColumnHeadersKorean(DataGridView dgv)
        {
            if (dgv == null) return;

            // 컬럼 헤더를 한글로 변경
            if (dgv.Columns["PURCHASEREQUESTID"] != null)
                dgv.Columns["PURCHASEREQUESTID"].HeaderText = "발주요청ID";
            if (dgv.Columns["RAWNAME"] != null)
                dgv.Columns["RAWNAME"].HeaderText = "원재료명";
            if (dgv.Columns["QUANTITY"] != null)
                dgv.Columns["QUANTITY"].HeaderText = "수량";
            if (dgv.Columns["UNITPRICEESTIMATE"] != null)
                dgv.Columns["UNITPRICEESTIMATE"].HeaderText = "단가";
            if (dgv.Columns["STATUS"] != null)
                dgv.Columns["STATUS"].HeaderText = "상태";
            if (dgv.Columns["REQUESTEDDATE"] != null)
                dgv.Columns["REQUESTEDDATE"].HeaderText = "요청일";
            if (dgv.Columns["EXPECTEDDELIVERYDATE"] != null)
                dgv.Columns["EXPECTEDDELIVERYDATE"].HeaderText = "예상납기일";
            if (dgv.Columns["REQUESTEDBYNAME"] != null)
                dgv.Columns["REQUESTEDBYNAME"].HeaderText = "요청자";
            if (dgv.Columns["APPROVEDBYNAME"] != null)
